...
async def _call_wikipedia_search(query: str, count: int) -> list[dict]:
    """Call Wikipedia search API to return educational snippets."""
    import re
    ...
    for item in search_results[:count]:
        ...
        # Remove HTML span tags from search snippets
        clean_snippet = re.sub(r'<[^>]*>', '', snippet)
        escaped_title = title.replace(' ', '_')
        results.append({
            "title": title,
            "url": f"https://en.wikipedia.org/wiki/{escaped_title}",
...
async def _call_search_api(query: str, count: int) -> list[dict]:
    ...
    if not SEARCH_API_KEY:
        logger.info("SEARCH_API_KEY not set; falling back to Wikipedia API.")
        try:
            return await _call_wikipedia_search(query, count)
        except Exception as exc:
            logger.error("Wikipedia search fallback failed: %s", exc)
            return []
